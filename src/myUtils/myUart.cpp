...
void MyUartHandler::Init(UartInstance instance, uint32_t baud_rate)
{
    switch (instance)
    {
    case SERIAL_USART1:
        ...
        break;
    case SERIAL_USART3:
        uhandle.huart1.Instance = USART3;
        break;
    case SERIAL_UART4:
        uhandle.huart1.Instance = UART4;
        break;
    case SERIAL_UART5:
        uhandle.huart1.Instance = UART5;
        break;
    }

    uhandle.huart1.Init.BaudRate               = baud_rate;
...
int MyUartHandler::PollReceive(uint8_t* buff, size_t size, uint32_t timeout)
{
    return HAL_UART_Receive(&uhandle.huart1, (uint8_t*)buff, size, timeout);
}

int MyUartHandler::StartRx()
{
    int status = 0;
    // Now start Rx
...
bool MyUartHandler::RxActive()
{
    return uhandle.rx_active;
}

int MyUartHandler::FlushRx()
{
    int status = 0;
#ifdef UART_RX_DOUBLE_BUFFER
...
int MyUartHandler::PollTx(uint8_t* buff, size_t size)
{
    return HAL_UART_Transmit(&uhandle.huart1, (uint8_t*)buff, size, 10);
}

int MyUartHandler::CheckError()
{
    return HAL_UART_GetError(&uhandle.huart1);
}

uint8_t MyUartHandler::PopRx()
{
#ifdef UART_RX_DOUBLE_BUFFER
